...
const jsonToTable = (json, classes) => {
  ...
  json.map(function(row) {
    ...
    columns.map(function(colName){
      bodyRows += `<td class="${(row["name"].split(" ")).join("-")}">` + row[colName] + '<td>';
    });
    bodyRows += `<button class="button-add-favorite ${(row["name"].split(" ")).join("-")}">Add "${row["name"]}" to my Favorites</button></tr>`;
  })
