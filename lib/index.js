...
const goToPlaylistShow = (id) => {
  fetch(`http://localhost:3000/api/v1/playlists/${id}`)
    .then(response => response.json())
    .then(data => populatePlaylistShow(data))
    .catch(error => console.error({error}));
}

const populatePlaylistShow = (id) => {
  window.location.href="playlist-show.html?="+`${id}`
}

$('#user-playlists').on('click', '.playlist-show-button', function() {
  var playlistId = this.parentElement.id;
  populatePlaylistShow(playlistId);
})
